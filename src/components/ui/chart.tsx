...
const ChartTooltipContent = React.forwardRef<HTMLDivElement, ChartTooltipContentProps>(
  (
    ...
    const tooltipLabel = React.useMemo(() => {
      ...
      const [item] = payload;
      const it = item as { dataKey?: string; name?: string };
      ...
      >
        ...
        <div className="grid gap-1.5">
          {payload.map((it, index) => {
            const item = it as { name?: string; dataKey?: string; value?: unknown; color?: string; payload?: { fill?: string } };
            ...
            return (
              ...
              >
                {formatter && item?.value !== undefined && item.name ? (
                  formatter(item.value, item.name, it, index, payload)
                ) : (
                  <>
                    {itemConfig?.icon ? (
                      <itemConfig.icon />
